...
Full RELRO가 적용되어 있으므로 got overwrite가 불가능하다. 따라서 ROPROP를 하기 위해서
1.  libc system 함수와 /bin/sh string의 위치
2.  stack canary 값
을 알아야 한다.

1을 얻기 위해서는 FSB를 통해 got의 값을 아무거나 읽으면 된다. 하지만 PIE가 적용되어 있어 got의 위치가 매번 바뀐다. 이는 return address를 선제적으로 알아내  code offset을 구함으로서 해결할 수 있다.
2 또한 
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbODg4NjE4MjU2LC0xNDA5OTQyNDE2LC0yMT
I4MDMyMjMyXX0=
-->