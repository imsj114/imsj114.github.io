...
중요한 점은 모든 payload가 0x00이 포함되지 않은 128 byte string이 되도록 만드는 것이다. `strlen()`함수의 값이 128이 되어야 FSB가 가능하기 때문에 null byte가 포함되면 안된다. 이게 어려운 이유는 stack canary의 least significant bit가 항상 0이기 때문인데, 이는 
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTIzNjUzMDI5MywtNDAzNjQ3MTIxLC0xND
A5OTQyNDE2LC0yMTI4MDMyMjMyXX0=
-->