...
먼저 1번이 생각보다 쉽지는 않았다. 가장 핵심이 되는 점은 `srand()`의 seed가 `time()` 이라는 사실이다. 서버의 시간을 정확히 알아야만 했다. 또한 seed를 알아도 C library의 `rand()` 값을 알기 위해서는 C의 사용이 필요했다.
...
2번 항목이 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMTczNzgwMTUsLTQzMTIzOTA1N119
-->