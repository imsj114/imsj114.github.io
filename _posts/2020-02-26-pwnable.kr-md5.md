...
BOF 과정에서 조금 헤맸는데, 처음에는 `fgets()`의 PLT를 사용해 /bin/sh를 입력하려고 했다. 하지만 함수의 입력으로 필요한 `stdin`을 인자로 넘겨줄 방법이 없다는 사실을 알게 되었다. 중요한 교훈이었다.


<!--stackedit_data:
eyJoaXN0b3J5IjpbMTAzOTMxNTg0OSwtNDMxMjM5MDU3XX0=
-->