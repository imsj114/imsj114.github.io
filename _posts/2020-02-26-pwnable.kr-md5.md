...
먼저 1번이 생각보다 쉽지는 않았다. 가장 핵심이 되는 점은 `srand()`의 seed가 `time()` 이라는 사실이다. 또한 

사실 내가 생각해낸 방법은 아니지만, 결국 가장 간단한 방법은 아무 id로 pwnable.kr에 ssh로 접속해 로컬에서 문제를 푸는 것이었다. python의 ctypes 라이브러리를 이용해 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzA5MjEyNDMyLC00MzEyMzkwNTddfQ==
-->